...
可运行AI版本：CS5以上
  ...
  脚本名：harmonizerCHN.jsx
  Name: harmonizerCHN.jsx;
...
var scriptName = 'HarmonizerCHN',
    settingFile = {
        name: scriptName + '__setting.json',
        folder: Folder.myDocuments + '/ZXC_AI_Scripts/'
    },
    useClippingMask = true;

var isUndo = false,
    win = new Window('dialog', scriptName+ '分布对象'+ ' \u00A9 ZXC', undefined);
...
var captionGutterX = groupGutterX.add('statictext', undefined, '水平间距:'),
    ...
    //valueGutterX = valueGutterX * 2.83465,
    positionX = groupGutterX.add('dropdownlist', [0, 0, 80, 25], ['左对齐', '垂直居中', '右对齐']);
...
var captionGutterY = groupGutterY.add('statictext', undefined, '垂直间距:'),
   ...
   // valueGutterY = valueGutterY * 2.83465,
    positionY = groupGutterY.add('dropdownlist', [0, 0, 80, 25], ['顶对齐', '水平居中', '底对齐']);
...
var toGroupCheckbox = groupCheckbox.add('checkbox', undefined, '群组'),
    randomOrderCheckbox = groupCheckbox.add('checkbox', [0, 0, 80, 20], '随机顺序');
...
with (panel.add('group')) {
    ...
    alignChildren = ['fill', 'fill'];
     ...
     with (add('group')) {
        ...
        add('statictext', undefined, '排列方式:');
        var reverseOrder = add('checkbox', undefined, '反向排列');
    }
     ...
     with (add('group')) {
        ...
        sortByY.helpTip = '从上到下排列';
        sortByX.helpTip = '从左到右排列';
        sortByS.helpTip = '从大到小排列，宽+高';
        sortByW.helpTip = '从宽到窄排列';
        sortByH.helpTip = '从高到矮排列';
    }
...
with (panel.add('group')) {
    ...
    alignChildren = ['fill', 'fill'];
     ...
     with (add('group')) {
        ...
        add('statictext', undefined, '间距数值为显示边缘间距，并为最小值，排列对象大小不同，按最大的尺寸分布',{multiline:true });
       }
...
var cancel = winButtons.add('button', undefined, '取消');
    cancel.helpTip = '按ESC键退出';
    cancel.onClick = function () { win.close(); }

var ok = winButtons.add('button', undefined, '运行');
...
function startAction() {
    ...
    var l = items.length, __rows = 0,
        gutter = {
            x: parseFloat(valueGutterX.text * 2.83465),
            y: parseFloat(valueGutterY.text * 2.83465)
        },
