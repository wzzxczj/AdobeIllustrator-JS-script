...
if (myVersion == 12 || myVersion == 11)
{
	...
	//if version is CS2
		...
		var CP_on = confirm("包括复合路径对象?");
		var includeLocked = confirm("包括锁定对象?");
...
} else if (myVersion > 12)
{
	...
	//--------------------------Dialog Box Code --------------------------------------------------------
	var dlg = new Window('dialog', '查找特定对象.Chlo汉化-大小-区域-宽度-高度-描边'); 
	dlg.location = [500,50];
	//																										[startX, StartY, EndX, EndY]
	dlg.alertBtnsPnl1 = dlg.add('group', undefined, 'Path Objects Size:');
	// Radio Buttons for Larger than, or smaller than
	(dlg.alertBtnsPnl1.selectLT = dlg.alertBtnsPnl1.add('radiobutton', [15,15,95,35], '小于' )).helpTip = "Select all  Path Items\nSmaller than\n the Slider Value."; 
	(dlg.alertBtnsPnl1.selectSS = dlg.alertBtnsPnl1.add('radiobutton', [15,15,75,35], '等于' )).helpTip = "Select all Path Items\nThe Same value as\n the Slider Value."; 
	(dlg.alertBtnsPnl1.selectGT = dlg.alertBtnsPnl1.add('radiobutton', [15,15,95,35], '大于' )).helpTip = "Select all Path Items\nLarger than \n the Slider Value."; 
	...
	(dlg.hasBtnsCb = dlg.alertBtnsPnl3.add('checkbox', [25,25,235,39], '包括复合路径的项目?')).helpTip=" Select items within compound paths too."; 
	(dlg.incLocked = dlg.alertBtnsPnl3.add('checkbox', [25,25,235,39], '包括锁定项?')).helpTip=" Note: This unlocks everything."; 
	...
	var topLocked = doc.layers[0].locked;
		if(OK==1){
			...
			*/
			...
			alert(doc.selection.length+" 选择路径 in "+minutes+" 分钟, "+Math.round(seconds)+" 秒.");
			//dispLayer.remove();
		} // end cancel test
} else {
	if (OK==2)
